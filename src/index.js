...
async function getWeatherDetails(location) {
  ...
  if (location) {
    var response = await fetch(`http://api.openweathermap.org/data/2.5/weather?q=${location}&APPID=0cdd23bc9251bc7da62c183bfc86ffde`, { mode: 'cors' });
  } else {
    response = await fetch(`http://api.openweathermap.org/data/2.5/weather?q=${inputValue}&APPID=0cdd23bc9251bc7da62c183bfc86ffde`, { mode: 'cors' });
  }
